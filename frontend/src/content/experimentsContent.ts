...
export const EXPERIMENTS_PAGE_COPY = {
  ...
  heroLead:
    "Здесь журнал инженерных решений. Для каждого узла пайплайна мы показываем: что пробовали, что вошло в прод, что дало ноль или регресс.",
  heroBadges: [
...
export const EXPERIMENTS_SECTIONS_COPY: ExpSectionCopy[] = [
  {
    ...
    shipped:
      "Дообученная нейросеть OpenFoodFacts (только класс «ценник»), кадр от 1536 точек, обучение с искажениями под нашу камеру и дообучение на датасетах.",
  },
  {
    id: "ocr",
    title: "OCR для ценников",
    blurb:
      "Сравнение локальных нейросетей, которые «читают» картинку, и пошаговая доводка запроса к модели и разбора её ответа.",
    ...
    finding:
      "Путь был от «не работает совсем» до почти идеального результата на нашем тесте — примерно за 10 аккуратных шагов. Сработала пара вещей: короткий чёткий запрос к модели плюс предсказуемая чистка её ответа кодом. Длинный подробный запрос только всё ухудшил — его откатили. Мелкие копейки в цене «без карты» — это предел качества исходного видео, а не нашего метода (даже глазами там чтото рассмотреть сложно).",
    shipped:
  ...
  },
  {
    ...
    finding:
      "На деле коды совершенно разные: где-то QR-ссылка, где-то непрозрачный квадратный код, где-то многоэтажный или линейный штрихкод. Единственное, что реально помогло их вскрыть, — искать код там, где он расположен по макету ценника. Сравнение «как было / как стало» показало честный потолок около 9% (увы) : дальше всё упирается в качество съёмки (смаз стирает мелкие элементы кода), и лечится это условиями записи и сбором из многих кадров, а не донастройкой сканеров.",
    shipped:
