...
- Цена «Без карты» = regular_price. Цена «С картой»/«По карте» (обычно крупнее) = loyalty_price.
- Маленькие верхние две цифры у цены — копейки: 114⁹⁹ → 114.99; 299⁰⁰ → 299.00.
- discount_amount — это метка скидки как напечатана: "-43%" или "-286₽" (в чёрном круге/куполе). Нет метки → "нет".
- price_discount — третья «акционная» цена с подписью «по карте от/до/при покупке N», иначе "нет".
